...
class SurveillanceSystem:
    def __init__(self, bot_token=None, chat_id=None, environment='development'):
        ...
        self.video_directory = "static/recorded_videos"  # Directory path for recorded videos
        self.fourcc = cv2.VideoWriter_fourcc(*"VP90")  # Define the codec for WebM format
    ...
    def callback(self, frame: np.ndarray) -> tuple:
        results = self.model(frame)[0]  # Run the YOLO model on the frame and get the results
        detections = sv.Detections.from_ultralytics(results)  # Convert results to Detections object
        detections = self.tracker.update_with_detections(detections)  # Update tracker with detections
        ...
        annotated_frame = self.box_annotator.annotate(frame.copy(), detections=detections)  # Annotate frame with bounding boxes
        annotated_frame = self.label_annotator.annotate(annotated_frame, detections=detections, labels=labels)  # Annotate frame with labels
    ...
    def send_telegram_message(self, message):
        ...
        """
        try:
            send_message_url = f"https://api.telegram.org/bot{self.bot_token}/sendMessage"  # URL for sending messages
            payload = {"chat_id": self.chat_id, "text": message}  # Payload for the POST request
    ...
    def write_metadata_to_file(self):
        ...
        self.metadata["total_duration"] = min((self.frame_count / self.frame_rate), self.record_duration)
        metadata_file = os.path.join(self.video_directory, f'{self.metadata["file_name"]}.json')
    ...
    def start_new_recording(self):
        ...
        self.metadata["start_time"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Add start time to metadata
    ...
    def process_frame(self, frame):
        detected_objects, processed_frame = self.callback(frame)  # Process the frame using the callback function
        if len(detected_objects) > 0:
            if not self.is_recording:
                ...
                object_names = [self.model.names[class_id] for class_id in class_ids]  # Get names of detected objects
                object_count = len(object_names)  # Count the number of detected objects
                object_word = "object" if object_count < 2 else "objects"  # Singular or plural
                ...
                self.send_telegram_message(message)  # Send the message
            ...
            for class_id, tracker_id, bbox in zip(detected_objects.class_id, detected_objects.tracker_id, detected_objects.xyxy):
                ...
                if self.metadata["detections"][tracker_id]["previous_position"] is not None:
                    previous_bbox = self.metadata["detections"][tracker_id]["previous_position"]
                ...
                self.metadata["detections"][tracker_id]["previous_position"] = current_bbox
        else:
        ...
        if self.is_recording:
            ...
            if elapsed_time >= self.record_duration:
                ...
                self.out.write(processed_frame)  # Write the processed frame to the output file
